...
def _is_missing_docker_image_error(error: str) -> bool:
    """Return True when Docker failed because the image is unavailable."""
    lowered = (error or "").lower()
    return "pull access denied" in lowered or "no such image" in lowered
...
class RendererAgent(BaseAgent):
    ...
    def __init__(
        ...
        *,
        ...
        auto_missing_image = False
        if use_docker is None:
            docker_ready = _docker_available()
            image_ready = docker_ready and _docker_image_exists(self._docker_image)
    ...
    def _render_one(
        ...
        *,
        ...
        if self._use_docker:
            ...
            if (
                proc_result["error"]
                and _is_missing_docker_image_error(proc_result["error"])
            ):
                self.logger.warning(
                    "Docker image '%s' unavailable while rendering %s. "
                    "Falling back to LOCAL subprocess (LLM scripts run without sandboxing).",
                    self._docker_image,
                    figure_id,
                )
                local_script_code = original_script_code.replace(
                    "/workspace/output", str(output_dir.resolve())
                )
                script_path.write_text(local_script_code, encoding="utf-8")
