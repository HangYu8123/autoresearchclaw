...
class GitHubClient:
    ...
    def __init__(self, token: str | None = None) -> None:
        ...
        self._search_interval: float = 2.0 if self._token else 6.0
    ...
    def _get(self, url: str, params: dict[str, str] | None = None) -> dict[str, Any] | None:
        ...
        try:
            ...
            if e.code == 401:
                logger.warning(
                    "GitHub API authentication failed (401). Clearing token, "
                    "degrading to unauthenticated mode."
                )
                self._token = ""
                self._auth_failed = True
                self._search_interval = 6.0
                return None
            if e.code == 403:
