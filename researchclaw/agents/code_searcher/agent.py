...
GITHUB_CODE_RESULTS_PER_QUERY = 5
...
class CodeSearchAgent:
    ...
    def search(
        ...
        for query in queries[1:self._max_code_searches + 1]:
            try:
                snippets = self._github.search_code(
                    query,
                    max_results=GITHUB_CODE_RESULTS_PER_QUERY,
                )
        ...
        for snippet in result.snippets_found[:GITHUB_CODE_RESULTS_PER_QUERY]:
            try:
