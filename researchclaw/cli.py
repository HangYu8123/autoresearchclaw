...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    for old_provider in ('provider: "openai-compatible"', 'provider: "deepseek"'):
        if old_provider in content:
            content = content.replace(old_provider, f'provider: "{provider}"', 1)
            break
    ...
    if provider == "acp":
        ...
        for old_base_url in (
            ...
            if old_base_url in content:
                content = content.replace(old_base_url, 'base_url: ""', 1)
                break
        for old_api_key_env in (
            'api_key_env: "OPENAI_API_KEY"',
            'api_key_env: "DEEPSEEK_API_KEY"',
        ):
            if old_api_key_env in content:
                content = content.replace(old_api_key_env, 'api_key_env: ""', 1)
                break
    else:
        ...
        for old_base_url in (
            ...
            if old_base_url in content:
                content = content.replace(old_base_url, f'base_url: "{base_url}"', 1)
                break
        if api_key_env:
            for old_api_key_env in (
                'api_key_env: "OPENAI_API_KEY"',
    ...
    if provider in _PROVIDER_MODELS:
        ...
        for old_fallbacks in old_fallback_blocks:
            if old_fallbacks in content:
