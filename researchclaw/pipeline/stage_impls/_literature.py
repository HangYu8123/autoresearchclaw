...
def _expand_search_queries(queries: list[str], topic: str) -> list[str]:
    ...
    max_total = len(queries) + 4
    terms = _extract_expand_terms(topic)

    def _add(candidate: str) -> None:
        candidate = candidate.strip()
        key = candidate.lower()
        if key and key not in seen and len(expanded) < max_total:
    ...
    short = " ".join(terms[:4]) if len(terms) >= 4 else " ".join(terms)
    _add(f"{short} survey")
    _add(f"{short} benchmark")
...
def _execute_literature_screen(
    ...
    *,
    ...
    # Sort by keyword relevance (desc) then recency (desc) so the most
    # relevant candidates survive the _MAX_CANDIDATES_CHARS truncation.
    filtered_rows.sort(
        key=lambda r: (r.get("keyword_overlap", 0), r.get("year", 0)),
        reverse=True,
    )
...
# ---------------------------------------------------------------------------
# PDF full-text enrichment for knowledge extraction
# ---------------------------------------------------------------------------

_MAX_PDF_TEXT_PER_PAPER = 5_000
_MAX_PDF_PAPERS = 3


def _enrich_shortlist_with_fulltext(
    rows: list[dict[str, Any]],
    stage_dir: Path,
) -> str:
    """Download and extract full text from top arXiv papers on the shortlist.

    Returns a text block of extracted content, capped at
    ``_MAX_PDF_PAPERS * _MAX_PDF_TEXT_PER_PAPER`` characters total.
    Falls back gracefully (returns empty string) on any failure.
    """
    try:
        from researchclaw.literature.arxiv_client import download_pdf
        from researchclaw.web.pdf_extractor import HAS_FITZ, PDFExtractor
    except Exception:  # noqa: BLE001
        return ""

    if not HAS_FITZ:
        logger.debug("PyMuPDF not installed — skipping PDF full-text enrichment")
        return ""

    # Identify shortlist rows with arXiv IDs
    arxiv_rows = [
        r for r in rows if r.get("arxiv_id") or _extract_arxiv_id(r.get("url", ""))
    ]
    if not arxiv_rows:
        return ""

    # Take the top N (already sorted by relevance_score from LLM screening)
    arxiv_rows = arxiv_rows[:_MAX_PDF_PAPERS]
    pdf_dir = stage_dir / "pdfs"
    pdf_dir.mkdir(parents=True, exist_ok=True)

    extractor = PDFExtractor(max_pages=20, extract_sections=True)
    snippets: list[str] = []

    for i, row in enumerate(arxiv_rows):
        arxiv_id = row.get("arxiv_id") or _extract_arxiv_id(row.get("url", ""))
        if not arxiv_id:
            continue
        title = row.get("title", arxiv_id)
        pdf_path: Path | None = None
        try:
            if i > 0:
                time.sleep(3.1)  # respect arXiv rate limits
            pdf_path = download_pdf(arxiv_id, dirpath=str(pdf_dir))
            if pdf_path is None:
                logger.info("PDF download failed for %s — skipping", arxiv_id)
                continue
            result = extractor.extract(pdf_path)
            if not result.success or not result.has_content:
                logger.info("PDF extraction failed for %s — skipping", arxiv_id)
                continue
            text = result.text[:_MAX_PDF_TEXT_PER_PAPER]
            snippets.append(
                f"--- Full-text snippet: {title} (arXiv:{arxiv_id}) ---\n{text}"
            )
            logger.info(
                "Extracted %d chars of full text for %s", len(text), arxiv_id
            )
        except Exception as exc:  # noqa: BLE001
            logger.warning("PDF enrichment failed for %s: %s", arxiv_id, exc)
        finally:
...
def _extract_arxiv_id(url: str) -> str:
    """Extract arXiv ID from an arXiv abs/pdf/html URL."""
...
def _execute_knowledge_extract(
    ...
    *,
    ...
    # PDF full-text enrichment for top arXiv papers (opt-in via config)
    if config.web_search.enable_pdf_extraction:
        shortlist_rows = _parse_jsonl_rows(shortlist)
        pdf_text = _enrich_shortlist_with_fulltext(shortlist_rows, stage_dir)
        if pdf_text:
