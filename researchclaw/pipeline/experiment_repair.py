...
def run_repair_loop(
    ...
    for cycle in range(1, max_cycles + 1):
        ...
        if not fixed_code:
            ...
            if _consecutive_gen_failures >= 2:
                logger.warning(
                    "[%s] Code generation failed %d consecutive times — "
                    "skipping remaining repair cycles",
                    run_id,
                    _consecutive_gen_failures,
                )
                break
            continue
...
def _repair_via_opencode(
    ...
    try:
        ...
        _oc_model = (
            getattr(_oc_cfg, "model", "")
            or getattr(config.llm, "primary_model", "")
            or ""
        )
