...
class TestArxiv:
    ...
    def test_search_arxiv_caps_requested_results(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """arXiv requests should stay small to avoid API 429s."""
        from unittest.mock import MagicMock
        ...
        papers = search_arxiv("test", limit=100)

        assert papers == []
        assert (
            _fake_arxiv.Search.call_args.kwargs["max_results"]
            == arxiv_client.ARXIV_MAX_RESULTS_PER_QUERY
        )

    def test_arxiv_client_page_size_is_small(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Client page_size controls the API max_results query parameter."""
        from unittest.mock import MagicMock
        from researchclaw.literature import arxiv_client

        _fake_arxiv = MagicMock()
        monkeypatch.setattr(arxiv_client, "arxiv", _fake_arxiv)
        monkeypatch.setattr(arxiv_client, "_client", None)

        _ = arxiv_client._get_client()

        _fake_arxiv.Client.assert_called_once_with(
            page_size=arxiv_client.ARXIV_MAX_RESULTS_PER_QUERY,
            delay_seconds=3.1,
            num_retries=3,
        )
