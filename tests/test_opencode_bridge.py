...
class TestOpenCodeBridge:
    ...
    def test_resolve_model_no_model_default(self):
        ...
        bridge = OpenCodeBridge()
        with pytest.raises(ValueError, match="OpenCode model is empty"):
            bridge._resolve_opencode_model()
    ...
    def test_generate_returns_error_when_model_empty_and_cli_available(self, tmp_path):
        bridge = OpenCodeBridge()
        ...
        assert not result.success
        assert "OpenCode model is empty" in result.error

    def test_generate_returns_error_on_cli_failure(self, tmp_path):
        bridge = OpenCodeBridge(
            model="gpt-5.2", max_retries=0, workspace_cleanup=True
        )
