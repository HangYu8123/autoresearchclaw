...
class TestGitHubClient:
    ...
    def test_search_interval_depends_on_auth_state(self):
        assert GitHubClient(token="ghp_test123")._search_interval == 2.0
        assert GitHubClient(token="")._search_interval == 6.0

    def test_401_clears_token_and_blocks_code_search(self):
        ...
        with patch("urllib.request.urlopen", side_effect=raise_401):
            assert client.search_repos("finite element") == []

        assert not client.has_token
        assert client._auth_failed is True
        assert client._search_interval == 6.0

        with patch.object(client, "_get", side_effect=AssertionError("should not call")):
            assert client.search_code("finite element") == []
...
class TestCodeSearchAgent:
    ...
    def test_search_with_mock_github(self, tmp_path):
        ...
        mock_github.search_code.assert_called()
        assert mock_github.search_code.call_args.kwargs["max_results"] == 1
