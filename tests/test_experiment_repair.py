...
```"""
    ...
    def test_repair_loop_stops_after_two_codegen_failures(self, tmp_path):
        """Permanent generation failures should fail fast after two cycles."""
        run_dir = self._make_run_dir(tmp_path, n_conditions=1)
        ...
        class FakeConfig:
            class experiment:
                time_budget_sec = 2400
                mode = "simulated"
                repair = ExperimentRepairConfig(
                    enabled=True, max_cycles=3, use_opencode=False
                )
                opencode = OpenCodeConfig(enabled=False)
                metric_key = "primary_metric"
        ...
        mock_llm = MagicMock()
        mock_resp = MagicMock()
        mock_resp.content = "No code blocks here."
        mock_llm.chat.return_value = mock_resp

        with patch("researchclaw.llm.create_llm_client") as mock_create_llm:
            mock_create_llm.return_value = mock_llm
            result = run_repair_loop(run_dir, FakeConfig(), "test-fail-fast")
    ...
    def test_repair_via_opencode_uses_primary_model_when_override_empty(self, tmp_path):
        """OpenCode repair should inherit config.llm.primary_model."""
        ...
        class FakeBridge:
            ...
            def generate(self, **kwargs):
                return SimpleNamespace(success=False, files={})
        ...
        with patch("researchclaw.pipeline.opencode_bridge.OpenCodeBridge", FakeBridge):
            result = _repair_via_opencode("repair prompt", {}, config, tmp_path, 1)
