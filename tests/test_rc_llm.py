...
def test_chat_uses_fallback_after_empty_response(monkeypatch: pytest.MonkeyPatch):
    calls: list[str] = []
    ...
    monkeypatch.setattr(LLMClient, "_call_with_retry", fake_call_with_retry)
    client = _make_client(
        primary_model="deepseek-v4-pro", fallback_models=["deepseek-v4-flash"]
    )
    response = client.chat([{"role": "user", "content": "x"}])
...
def test_chat_allows_empty_content_filter_response(monkeypatch: pytest.MonkeyPatch):
    ...
    def fake_call_with_retry(
        ...
        _ = (self, messages, max_tokens, temperature, json_mode)
        calls.append(model)
        return LLMResponse(content="", model=model, finish_reason="content_filter")
