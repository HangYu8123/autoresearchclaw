...
class TestRendererAgent:
    def test_auto_disables_docker_when_image_missing(self):
        from researchclaw.agents.figure_agent.renderer import RendererAgent

        with mock.patch(
            "researchclaw.agents.figure_agent.renderer._docker_available",
            return_value=True,
        ), mock.patch(
            "researchclaw.agents.figure_agent.renderer._docker_image_exists",
            return_value=False,
        ):
            agent = RendererAgent(
                _FakeLLM(), use_docker=None, docker_image="missing/image:latest"
            )

        assert agent._use_docker is False

    def test_docker_missing_image_falls_back_to_local(self, tmp_path):
        from researchclaw.agents.figure_agent.renderer import RendererAgent
        ...
        output_file = output_dir / "fig_fallback.png"
        ...
        with mock.patch.object(
            agent,
            "_execute_in_docker",
            return_value={"error": "pull access denied for missing/image"},
        ) as docker_mock:
            with mock.patch.object(
                ...
                result = agent.execute({
                    "scripts": [{
                        "figure_id": "fig_fallback",
                        "script": "open('/workspace/output/fig_fallback.png', 'wb').write(b'0' * 2048)",
                        "output_filename": "fig_fallback.png",
                    }],
        ...
        script_path = output_dir / "scripts" / "fig_fallback.py"
        assert "/workspace/output" not in script_path.read_text()
